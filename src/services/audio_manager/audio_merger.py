...
class AudioMerger:
    ...
    def merge_audio(
        ...
        try:
            ...
            # Load content audio
            logger.info("Loading content audio...")
            content_audio = AudioSegment.from_mp3(
                io.BytesIO(content_audio_bytes)
            )

            # Normalize content audio format (MP3, 44100 Hz, 128 kbps)
            ...
            # Export as MP3
            logger.info("Exporting merged audio to MP3...")
            output = io.BytesIO()
            merged_audio.export(
                output,
                format="mp3",
            ...
            logger.info(
                f"Merged audio complete: {len(merged_bytes)} bytes, "
